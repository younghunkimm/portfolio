            ...
            <Layout>
                ...
                <Section title="쿠폰팝 (CouponPop)">
                    <ProjectOverview
                        description={
                            <>
                                CouponPop은 사용자의 <b>현재 위치 기반</b>으로
                                주변 매장에서 제공하는{" "}
                                ...
                                <br />
                                <br />
                                모바일/웹 공통 알림, 피크 트래픽 대응, MSA 전환
                                이후 JOIN 문제 해결까지 포함해{" "}
                                <b>신뢰성과 운영성</b>을 중심으로 설계했습니다.
                            </>
                        }
                        metrics={[
...
public void handle(CouponUsageStatsFcmSendMessage payload) {
    ...
    try {
        ...
        throw e;
                                    ...
                                    AmqpRejectAndDontRequeueException으로
                                    래핑해서 throw → 재시도 정책에서 제외시키고,
                                    즉시 DLQ 이동
                                    <br />- 나머지(RetryableFcmException 포함) →
                                    그냥 예외 던짐 → Spring Retry가 설정한
                                    정책에 따라 재시도
                                </>,
                            ]}
                        />
                        <BulletList
                            ...
                            items={[
                                <>
                                    지수 백오프 + 최대 5회 재시도
                                    <br />
                                    - 재시도 가능한 예외 → 1s, 2s, 4s, 8s, … 식
                                    지수 백오프로 재시도
                                    <br />
                                    - 최대 횟수(5번) 소진 시 →
                                    RejectAndDontRequeueRecoverer →
                                    NACK(requeue=false) → DLX로 이동
                                    <br />- AmqpRejectAndDontRequeueException →
                                    아예 재시도 정책에서 제외 → 바로 DLX
                                    <CodeBlock language="java">
                                        {`// AmqpRejectAndDontRequeueException은 즉시 DLQ로 보내도록 재시도 대상에서 제외
Map<Class<? extends Throwable>, Boolean> retryableExceptions = new HashMap<>();
retryableExceptions.put(AmqpRejectAndDontRequeueException.class, false);
...
public void handleDlq(CouponUsageStatsFcmSendMessage payload, Message message) {
    ...
    slackService.sendMessage("쿠폰 사용 통계 FCM DLQ 적재", slackData);
                                    ...
                                    DLQ 적재 시점에 에러 정보를 Slack으로
                                    전송해서 알림 유실이 장기화되기 전에 바로
                                    인지할 수 있도록 처리
                                </>,
                            ]}
                        />
                ...
                <Section title="MSA ERD 구성">
                    <div className="grid gap-6 md:grid-cols-3 sm:grid-cols-2">
                        ...
                        />
                        <ImageBox
                            src={ErdMSAStoresImage}
                            label="MSA Stores ERD"
                        />
                        <ImageBox
